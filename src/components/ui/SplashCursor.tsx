...
export default function SplashCursor({
  ...
  const enablesplashcursor = signal(true);
  const isPrefersReducedMotionStore = signal(false);
  isPrefersReducedMotionStore.value = useStore(isPrefersReducedMotion);
  ...
  useEffect(() => {
    ...
    if (isPrefersReducedMotionStore.value) {
      config.SPLAT_FORCE = 0; // disable splats
      config.PAUSED = true; // optional: pause simulation
      enablesplashcursor.value = false;
      return;
    } else {
      config.SPLAT_FORCE = 6000;
      config.PAUSED = false;
      enablesplashcursor.value = true;
  ...
  return enablesplashcursor.value ? (
    <div
      style={{
        position: "fixed",
        top: 0,
