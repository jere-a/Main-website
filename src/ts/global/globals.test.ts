...
import { language, capitalize } from './globals.ts';

var lang_codes = ["af", "af-NA", "af-ZA", "agq", "agq-CM", "ak", "ak-GH", "am",
...
test('Test if language return right language code', () => {
  for (let code in lang_codes) {
    if (code.split('-')[0].length >= 3 || code.length === 3) continue;
    expect(language(false, code)).toBe(code.substring(0, 2));
    expect(language(true, code)).toBe('fi');
  }
})
...
test('Capitalization', () => {
  ...
  expect(capitalize('-- comment')).toBe('-- comment');
})
